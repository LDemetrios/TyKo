...
import org.ldemetrios.tyko.model.TSetDocument
import org.ldemetrios.tyko.model.UnrecognizedSetRule
...
val SKIP = setOf(
    "ops-equality", // https://github.com/typst/typst/issues/7799

    "curve-stroke-gradient-sharp", // Float precision issues, probably (dist = 110)
...
private class TypstSharedFfiRunner {
    ...
    private val outputDir = File(repoRoot, "tests/typst-shared-ffi/build/typst-shared-ffi-output")
    ...
    fun runTest(test: TestCase) {
        ...
        withTestContext(test) { context, main ->
            ...
            if (test.attrs.eval) {
                val eval = runtime.evalMainWarnedRaw(
                    context,
                    main,
                    fonts = fonts,
                    stdlib = library,
    ...
    private fun roundTripCompare(
        ...
        val result = runtime.detachedEvalRaw(
            test.content,
            library = library,
            fonts = fonts,
        ...
        val evaled = when (result) {
            ...
            is RResult.Err -> {
                if (result.error.any {
                        it.message.contains("cannot access file system") ||
                                it.message.contains("unable to get the current date")
                    }) {
                    return
                }
